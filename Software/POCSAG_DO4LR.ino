...
void init_tx(void){
  Serial.begin(9600);
  if(rf22.init()){
    Serial.println("ezPOC8 init OK");
    }
  else{
    Serial.println("ezPOC8 init FAILED");
    }
  ...
  Serial.println("<Adresse> <source> <Wiederholungen> <Nachricht>"); // Leerzeichen müssen vorhanden sein Source muss zwischen 0 und 3 sein, Wiederholungen zwischen 1 und 9, Die Nachricht darf max. 'anz' lang sein
   ...
   if(state == -1){
    ...
    delay(10000);
      ...
      for(int l= -1; l < repeat; l++){
        digitalWrite(TX_LED, HIGH);
        digitalWrite(WDP, HIGH);//Watchdog aktivieren
        ...
        delay(1000);
        digitalWrite(TX_LED, LOW);
        digitalWrite(WDP, LOW); //nicht vergessen den Watchdog wieder zu deaktivieren
        }
      Serial.println("Sendezyklus beendet");
      Serial.println("*~*");
      }
